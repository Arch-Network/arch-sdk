...
#[cfg(target_os = "solana")]
pub fn sol_log_data(chunks: &[&[u8]]) {
    unsafe { crate::syscalls::sol_log_data(chunks.as_ptr() as *const u8, chunks.len() as u64) };
}

#[cfg(not(target_os = "solana"))]
pub fn sol_log_data(chunks: &[&[u8]]) {
    crate::program_stubs::_sol_log_data(chunks.as_ptr() as *const u8, chunks.len() as u64);
}
